...
def plot_models(csv_paths, rows: int | None, cols: int | None, out_path: Path) -> None:
    ...
    elif rows is None:                     # --cols only
        rows = math.ceil(n / cols)
    elif cols is None:                     # --rows only
    ...
    for idx, csv_file in enumerate(csv_paths):
        ...
        ax.set_xlabel("Actual Ki-67 [%]", fontsize=24)
        ax.set_ylabel("Predicted Ki-67 [%]", fontsize=24)
        ax.set_xlim(-5, 105)
        ax.set_ylim(-5, 105)
        ...
        ax.set_title(letters[idx], loc="center", fontsize=26, fontweight="bold")
...
if __name__ == "__main__":
    ...
    if args.csvs == []:
        print(
            ...
            "Example (BCData – eight runs in a 2×4 grid):\n\n"
            "BCData:\n"
